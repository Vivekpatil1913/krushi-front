...
const ConsultancyBanner = () => {
  ...
  const loadBanners = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`${API_BASE_URL}/banners/page/Consultancy`)
  ...
  const StaticBanner = () => (
    <div className="cb-banner-wrapper">
      ...
      <div className="cb-banner-content">
        <motion.div
          ...
          className="cb-text-container"
        >
          <motion.h1
            className="cb-main-title"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          >
             Strategic <span className="cb-title-emphasis">Solutions</span> for Agricultural Growth
          </motion.h1>
          ...
          >
            Expert guidance to optimize your agricultural operations and maximize productivity
          </motion.p>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
          >
            <span className="feature-icon"><PiStrategyBold /></span>
            Our Services
            <FiArrowRight className="cb-arrow-icon" />
          </motion.button>
        </motion.div>
      </div> 
    </div>
